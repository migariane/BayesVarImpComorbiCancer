---
title: "MI - Lung cancer: Early Stages"
...
# Reading the data
survdata <- read.csv("LungImputed.csv", header = TRUE, stringsAsFactors=FALSE)
head(survdata)
dim(survdata)
colnames(survdata)
...
# Stage as numeric
survdataMI$stage <- as.factor(survdataMI$stage)

survdataMI$stage <- as.numeric(survdataMI$stage)

colnames(survdataMI)

survdataMI <- survdataMI[complete.cases(survdataMI),]
...
for(k in 1:5){
  ...
  # Extracting early stage patients
  ind.early <- which(survdataMI$stage[ ((k-1)*n+1):(k*n) ] == 1)
  
  survdatak <- survdataMI[ ((k-1)*n+1):(k*n), ][ind.early,]
  
  # Survival times
  survdatak$time <- as.numeric(time_length(interval(dmy(survdatak$datainci), dmy(survdatak$datefoll)), "year"))
  
  # log Survival times and status
  y <- Surv(log(survdatak$time), event=survdatak$status)
  
  # index containing the variables of interest
  indX <- (1:ncol(survdatak))[!colnames(survdatak)%in%c("status","datainci","datefoll","time", "stage")] 
...
for(k in 1:5){
  ...
  data_plot_names
  data_plot_names <- c("Connective disease", "Vascular disease", "Pulmonary disease", "Diabetes",
                       "Gender", "Infarct", "Heart failure", "Cerebrovascular disease",
                       "Dementia", "Current smoker", "Previous smoker", "Renal disease",
                       "Liver disease", "Age") # Fix labels
  data_plot_names
